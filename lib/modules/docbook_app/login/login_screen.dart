      ...
      listener: (BuildContext context, state) {
        if(state is LoginSuccessState){
          ...
          CasheHelper.saveData(key: 'token', value: state.loginModel.token).then((value) {
            ...
            DocLoginCubit.get(context).getDoctorAppointment(userid, date);
            ...
            if (state.loginModel.role![0] == "doctor") {
              DocLoginCubit.get(context).getDoctorAppointment(userid, date).then((value){
                DocLoginCubit.get(context).getDoctorData(userid);
