...
class DocLoginCubit extends Cubit<DocLoginStates> {
  ...
  void updateUser({
    ...
    await DioHelper.putData(
      ...
      showToast(text: 'Cant update now ', state:ToastStates.ERROR);
      print(error.toString());
        ...
        value.data.forEach((element) {
          ...
          if (DateTime.tryParse(element["time"])!.isAfter(
              ...
              DateTime.tryParse(element["time"]).toString()==
                  '${DateFormat("yyyy-MM-dd").format(DateTime.now()).toString()} 00:00:00.000'){
            appointments.add(UserAppointment.fromJson(element));
          } else {
            //print('-----');
            //print(DateFormat("yyyy-MM-dd").format(DateTime.now()).toString());
            //print(element['start']);//6:00am
            //print(element['start'].toString()); //6:00am
            //print('2--------');
            //print(DateTime.tryParse(element["time"]));
            //print(DateFormat.jm().format(DateTime.now()).toString());
            //print(DateFormat.Hm().format(DateTime.now()).toString());
            //print(DateFormat.Hm().format(element['start']).toString());
            //print(DateFormat.Hm().format(element['start']));
            //print('3------');
            ...
            //print(DateTime.tryParse(element["time"])!.day);
            print("before");
          }
        });
  ...
  Future<void> getDoctorAppointment(String? id, String date) async {
    ...
    if (doctor == true) {
      ...
      await DioHelper.getData(
        ...
        docAppointment.sort((a, b) { //sorting in ascending order
          return DateTime.parse(a.time!).compareTo(DateTime.parse(b.time!));
        });
